...
import * as A from './art.js?v=1.8.2';
import * as UI from './ui.js';
import * as SFX from './audio.js';
import * as INV from './inventory.js';
import { inv } from './inventory.js';
import * as SAVE from './save.js';
import { drawMenuScene, resetMenuScene } from './menu.js';
...
function syncPixelGrid() {
  ...
  lighting.rebuild();
  resetMenuScene(); // 菜单那一幕自带一套缓冲，也得按新倍率重搭
  A.clearRingCache();
  const id = area.id;
  dropAreaCache();
  area = getArea(id);
...
/**
 * 离开主菜单。DOM 那半边交给 UI，canvas 那半边把整幕丢掉 ——
 * 收容观察间自带一整套 SCALE×N 网格上的静态层与烘焙光贴图，
 * 4K 下体量和一个游戏区域相当，留着白占显存。
 */
function leaveMenu() {
  UI.hideMenu();
  resetMenuScene();
