 ...
 * 所以整幕在一个 320×180 的"场景空间"里搭，最后 setTransform(2) 一次性
 * 铺满 640×360。静态光源与裁剪遮罩照例烘焙，每帧只做一次 drawImage。
 */

import { VIEW_W, VIEW_H, HW, HH, TILE_W, TILE_Z } from './config.js';
...
import { clamp, flicker } from './util.js';
...
export function drawMenuScene(ctx, t) {
  ...
  // 静态层
  ctx.drawImage(a.statics.img, Math.round(cam.x - a.statics.ox), Math.round(cam.y - a.statics.oy));
  ...
  for (const it of items) {
    ...
    if (p.chamber) {
      ...
      ctx.drawImage(p.chamber.back.img, cx - p.chamber.back.ox, cy - p.chamber.back.oy);
      A.drawSpecimen(ctx, cx + 1, cy - A.CHAMBER_FLOOR_Z * TILE_Z, { t, press });
      ctx.drawImage(p.chamber.front.img, cx - p.chamber.front.ox, cy - p.chamber.front.oy);
      continue;
    }
    const s = sc(cam, p.x, p.y);
    ctx.drawImage(p.s.img, Math.round(s.x - p.s.ox), Math.round(s.y - p.s.oy));
  }
