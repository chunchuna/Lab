 ...
 * 美术网格（纹素）
 *
 * 画布仍然是 640N×360N，和设备像素 1:1 —— 清晰度不动。变的是**美术本身**
 * 落在多粗的格子上：角色、道具、地板与墙这些"填色"先画在一张
 * 640×360 那一档的离屏画布上，再关掉插值整数倍贴回来，于是每个纹素
 * 都是一块边长 N 个设备像素的实心方块。
 *
 * 这跟"先画小再吹大"不是一回事：吹大的是**素材**，光照、雨、辉光、渐变、
 * 准星、DOM 文字全都还画在 640N 的网格上，一点没降。想要的正是这个反差 ——
 * 像素质感的美术 + 设备像素级的光。
 *
 * ART_TEXEL 是"一个逻辑像素里放几个纹素"：1 = 原本 640×360 的像素尺度，
 * 2 = 细一倍。想调手感改这一个数就行。
 * ------------------------------------------------------------------ */
export const ART_TEXEL = 1;

/**
 * 美术离屏画布的倍率。要求 pixelScale() 能被它整除 —— 除不尽的话，
 * 一个纹素在屏幕上就会一会儿占 2 个设备像素、一会儿占 3 个，格子宽窄不一。
 */
export function artScale() {
  const n = pixelScale();
  let a = Math.max(1, Math.min(n, Math.round(pixBoost * ART_TEXEL)));
  while (a < n && n % a !== 0) a++;
  return a;
}
